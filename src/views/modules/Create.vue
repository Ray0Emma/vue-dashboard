...
const submit = () => {
  const data = {
    ...
    groupe: departement.groupe,
    semester: departement.semester,
  };
    DepartementDataService.createData("modules", data)
    .then((response) => {
      departement.id = response.data.id;
      departement.name = response.data.name;
      departement.groupe = response.data.type;
      departement.semester = response.data.semester;
      router.push("modules");
    })
    .catch((e) => {
      alert(e);
